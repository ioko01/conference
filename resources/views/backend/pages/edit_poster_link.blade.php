        ...
        <div class="card-content">
            ...
            <div class="card-body">
                <form method="POST" enctype="multipart/form-data"
                    action="{{ route('backend.oral.link.update', $link_poster->id) }}" class="mb-3">
                    @csrf
                    @method('PUT')
                    <div class="row">
                        <div class="col-md-4 mb-3">
                            ...
                            <select name="faculty_id" id="faculty_id"
                                ...
                                @forelse ($faculties as $faculty)
                                    ...
                                    @endif
                                    <option value="{{ $faculty->id }}" @if ($link_poster->faculty_id == $faculty->id) selected @endif>
                                        {{ $faculty->name }}</option>
                                @empty
                    ...
                    <div class="row">
                        <div class="col-md-4 mb-3">
                            ...
                            <input value="{{ $link_poster->room }}" type="text" name="room" id="room"
                    ...
                    <div class="row">
                        <div class="col-md-4 mb-3">
                            ...
                            <input value="{{ $link_poster->link }}" type="text" name="link" id="link"
                            ...
                            <div class="mb-3">
                                ...
                                <div class="mb-3" style="position: relative">
                                    <label class="label-type-file mb-0 @error('file') is-invalid @enderror"
                                        for="file">{{ $link_poster->name ? $link_poster->name : 'ไม่ได้เลือกไฟล์ใด' }}</label>
                                    <input type="hidden" value="{{ $link_poster->name }}" name="name_file" id="name_file">
            ...
            <div class="card-body text-xs">
                <div class="table-responsive">
                    <table class="table table-striped">
                        ...
                        <tbody>
                            @forelse ($link_posters as $key => $link_poster)
                                <tr>
                                    <td>{{ $key + 1 }}</td>
                                    <td>{{ $link_poster->room }}</td>
                                    <td><a href="{{ $link_poster->link }}" target="_blank"
                                            rel="noopener noreferrer">{{ $link_poster->link }}</a></td>
                                    <td class="text-center">
                                        <button class="btn btn-link"
                                            onclick="open_modal_default('#modal', 'sm', 'QR Code', {{ $link_poster }})"><i
                                                class="fa fa-search-plus"> </i> ดูภาพ</button>
                                    </td>
                                    <td>{{ $link_poster->faculty_name }}</td>
                                    <td class="text-right"><a href="{{ route('backend.oral.link.edit', $link_poster->id) }}"
                                            class="btn btn-sm text-white btn-warning rounded-0"><i class="fa fa-edit"></i>
                                            แก้ไข</a>
                                    </td>
                                    <td><button
                                            onclick="open_modal('{{ $link_poster->room }}', '{{ route('backend.oral.link.delete', $link_poster->id) }}')"
