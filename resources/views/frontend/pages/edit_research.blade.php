                ...
                @foreach ($researchs as $research)
                    ...
                    @enderror"
                        ...
                        <div class="mb-4">
                            ...
                            is-invalid
                        @enderror"
                                    @if (isset(explode('|', $research->presenter)[0])) value="{{ explode('|', $research->presenter)[0] }}" @endif autocomplete="presenters[0]">
                            ...
                            <div class="mb-4">
                                ...
                                <input type="text" id="presenters[]" name="presenters[]" class="form-control w-100"
                                    @if (isset(explode('|', $research->presenter)[1])) value="{{ explode('|', $research->presenter)[1] }}" @endif autocomplete="presenters[1]">
                            </div>
                            <div class="mb-4">
                                <span>3.&nbsp;</span>
                                <input type="text" id="presenters[]" name="presenters[]" class="form-control w-100"
                                    @if (isset(explode('|', $research->presenter)[2])) value="{{ explode('|', $research->presenter)[2] }}" @endif autocomplete="presenters[2]">
                            </div>
                            <div class="mb-4">
                                <span>4.&nbsp;</span>
                                <input type="text" id="presenters[]" name="presenters[]" class="form-control w-100"
                                    @if (isset(explode('|', $research->presenter)[3])) value="{{ explode('|', $research->presenter)[3] }}" @endif autocomplete="presenters[3]">
                            </div>
                            <div class="mb-4">
                                <span>5.&nbsp;</span>
                                <input type="text" id="presenters[]" name="presenters[]" class="form-control w-100"
                                    @if (isset(explode('|', $research->presenter)[4])) value="{{ explode('|', $research->presenter)[4] }}" @endif autocomplete="presenters[4]">
                            </div>
                    ...
                    @enderror"
                                ...
                                @foreach ($faculties as $faculty)
                                    <option value="{{ $faculty->id }}" @if ($faculty->id == $research->faculty_id)
                                        selected
                                @endif>
                                {{ $faculty->name }}
                                </option>
                ...
                @foreach ($presents as $present)
                    <div class="form-check">
                        <input class="form-check-input" type="radio" name="present_id" id="present_{{ $present->id }}"
                            value="{{ $present->id }}" @if (!$research->present_id)
                        @if ($present->id == $presents->first()->id)
                            checked
                        @endif
                    @elseif ($present->id == $research->present_id)
                        checked
                @endif>
            ...
            @foreach ($tips as $tip)
                <div class="tips-box py-5">
                    ...
                    <div class="content"><strong>{{ $tip->head }}</strong><br /><span><textarea readonly
                                style="width: 100%;" class="txt-tips autosize">{{ $tip->detail }}</textarea></span></div>
                </div>
            @endforeach
        </div>
    </div>
